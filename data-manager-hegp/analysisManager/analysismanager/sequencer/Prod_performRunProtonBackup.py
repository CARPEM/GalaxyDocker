...
def backupAllCovariantAnalysis(coverageAnalysisDict,pathtocov,sshProton,checksumcov,failedFileToREDO):
	...
	for Analysis,data in coverageAnalysisDict.iteritems():
		...
		logger.debug(bamchecksum)
		logger.debug( "check if the file "+thisAnalysis+"/"+data['bcmatrix'].split("/")[-1])
		...
		if os.path.exists(thisAnalysis+"/"+data['bcmatrix'].split("/")[-1]):
			...
			process1 = subprocess.Popen(bashCommand1.split(), stdout=subprocess.PIPE)
			copybam=process1.communicate()[0].split(" ")[0]			
			logger.debug("Check copy")
			if	(copybam==bamchecksum) :
				logger.debug("COMPLETE:"+str(bamchecksum))
			...
			try :
				...
				logger.debug( "this file failed to upload"+str(thisAnalysis))
				failedFileToREDO.write("this file failed to upload :"+str(thisAnalysis)+"/"+data['bcmatrix'].split("/")[-1]+"\t"+str(bamchecksum)+"\n")
				scp.close()
				scp=SCPClient(sshProton.get_transport())
				CopydataAgain(data['bcmatrix'],thisAnalysis,data['bcmatrix'].split("/")[-1],bamchecksum,checksumcov,1,sshProton)
			...
			jobdone=False
			while jobdone!=True:
				logger.debug("This prints once a 10sec. and wait for data")
				time.sleep(10) 
				jobdone=True
...
def performWholeRunBackup(runNametobackup):
	##create here an object for the data and check the status
	#add a pointer to the object at each iteration
	logger.debug("##########################")
	...
	ssh_stdin_bam, ssh_IonXpress_stdout_bam, ssh_stderr_bam = sshProton.exec_command(" ls "+absolutpath+"/"+runNametobackup.rstrip()+"/download_links/IonXpress*bam")
	bamfileList=[] #contains bamfile list
	for bamfile in ssh_IonXpress_stdout_bam.readlines() :
	...
	logger.info(pathtoResult+"/checksumbam.txt")
